...
def nfvipop_from_internal(host):
  ...
  if "bandwidth" in host["capabilities"]:
    ...
    bw = 0
  mecCapable = False
  if "mec" in host["capabilities"]:
    mecCapable = host["capabilities"]["mec"]
...
def vnf_to_internal(ext):
  ...
  if "CP" in ext:
    internal["CP"] = copy.deepcopy(ext["CP"])

  if "location" in ext:
    internal["location"] = copy.deepcopy(ext["location"])

  mecApp = False
  if "mec" in ext["requirements"]:
    mecApp = ext["requirements"]["mec"]
...
def vnf_from_internal(internal):
  ...
  if "CP" in internal:
    ext["CP"] = copy.deepcopy(internal["CP"])

  mecApp = False
  if "mec" in internal["requirements"]:
    mecApp = internal["requirements"]["mec"]
...
def network_service_to_internal(nsd):
  ...
  for vl in nsd["VNFLinks"]:
    if float(vl["traversal_probability"]) < 1.0 and vl["destination"] != "None":
...
def preprocess_external_request(req):
  """Processes external request to augment it with the fields that are missing,
  i.e., source-destination per VNFLink, taking into account CP information.

  In R2, source and dst have been removed from VLs. Instead:
  - Each VL only has an ID
  - Each VNF has CPs, each in turn associated with a VL or a SAP
  
  Handling VLs
  ------------
  In order to find out the endpoints of a VL, we follow this procedure:
  - Loop over all VNFs
  - For each VNF's CP:
      + Extract its corresponding VNF link id
      + Search in VNFLinks for the link with the above id
      + If the link is found and doesn't have already a src, set VNFLink.source = VNF id
      + Else, set VNFLink.destination = VNF id
      
  This assumes that a VNFlink has at most 2 CPs associated with it. At the end
  of the procedures, each VNFLink will have zero, one, or 2 endpoints.
  
  Handling SAPs
  -------------
  The purpose is to augment a VNF with location information. This information,
  however, is encoded in a SAP and not in a VNF information element. This is how
  we handle this:
  - For each SAP, find its CP id
  - Loop over all VNFs. For each VNF v:
      + Find the CP id with matches that of the SAP
      + Copy the SAP's location info field to the VNF
  
  Assumption: It may happen that multiple SAPs with different locations are
  associated with a CP of a VNF. In this case, the first one applies.
  """

  # Augment VLs with source-destination information
