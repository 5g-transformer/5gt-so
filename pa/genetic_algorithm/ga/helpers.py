...
def check_host_capacity_constraint(solution, h):
  ...
  for r in solution["resource_types"]:
    if get_host_available_capacity(solution, h, r) < 0:
      logging.warn(r + " capacity exceeded: " + str(get_host_available_capacity(solution, h, r)))
      return False
...
def check_if_there_is_space(solution, v):
  ...
  for h in solution["hosts"]:
    ...
    for r in solution["resource_types"]:
      if get_host_available_capacity(solution, h, r) < v["requirements"][r]:
...
def check_global_feasibility(scenario):
  """Check if there is overall capacity to host the requested VNFFG.
...
def get_solution_delay(solution, s, processing_delay=False):
  ...
  for vedge in solution["vnf_edges"]:
    ...
    if hedge:
      hlat = hedge["delay"]
      #print "host edge found:" + str(hedge)
    else:
      ...
      #print "host edge not found:" + vedge["source"] + "-" + vedge["target"]
      hlat = 0.0
    ...
    #print "Edge contributing to latency by " + str(probability) + "*" + str(hlat) + "=" + str(probability*hlat)
...
def get_used_host_links(solution):
  ...
  for ve in solution["vnf_edges"]:
    ...
    if he is None:
      ...
      v1 = filter(lambda x: x.get("vnf_name") == ve["source"], solution["vnfs"])[0]
      v2 = filter(lambda x: x.get("vnf_name") == ve["target"], solution["vnfs"])[0]
      
      # find source host
      if v1["place_at"][0] == v2["place_at"][0]:
        he = {"capacity": 100000, "delay": 0, "source": v1["place_at"][0], "target": v1["place_at"][0]}
      else:
        print "Null edge:" + v1["place_at"][0] + "->" + v2["place_at"][0]
    if he not in hedges:
      hedges.append(he)
  return hedges

def add_missing_links(scenario):
  """For each host link, add a symmetric link in the opposite direction.
  
  This is necessary if the scenario file assumes undirected links.
  """
  new_edges = []
